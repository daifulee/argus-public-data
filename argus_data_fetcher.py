...
def fetch_today_row() -> dict:
    ...
    pmi_val, pmi_period, pmi_source = _fetch_pmi_4layer_defense()
    if pmi_val is not None:
        row['PMI'] = pmi_val
        # 🌟 v2.12 (S69 #5): PMI source 결정적 본질 (1차 success = LIVE)
        row['PMI_source'] = pmi_source  # "tradingeconomics" / "usslind_proxy" / etc.
        print(f"  🌟 PMI: {pmi_val} ({pmi_period}, source={pmi_source}) — 4중 방어 1차 성공")
    else:
    ...
    if fg_score is not None:
        ...
        # 🌟 v2.12 (S69 #5): F&G source 결정적 본질
        row['F_G_source'] = fg_source  # "cnn_api" / "cnn_html" / "argus_proxy"
        print(f"  🌟 F&G: {fg_score:.1f} ({fg_rating}, source={fg_source})")
    else:
        # 4차 fallback — ffill 단계에서 자동 처리
        # 🌟 v2.12 (S69 #5): 4차 fallback = ffill source 명시
        row['F_G_source'] = "ffill"
        print(f"  🟡 F&G: 4중 방어 1~3차 실패 → ffill carry-forward (source={fg_source})")
...
def print_quality(df: pd.DataFrame):
    ...
    for col in ["VIX","VIX3M","VIX_VIX3M_ratio","TNX","DFII10","DXY","WTI","PMI","T10YIE","OAS_HY","MOVE","ICSA","CCSA","UMCSENT","F_G_Score"]:
...
def main():
    t0 = time.time()
    print(f"🦅 ARGUS DATA FETCHER v3.1 — {datetime.now(KST).strftime('%Y-%m-%d %H:%M KST')}")
